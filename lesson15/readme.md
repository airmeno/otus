...
Данное решение по сути возвращает размещение файлов на места по умолчанию, что соотвествует контектсу безопасности SELinux для Bind (named).
...
Элементов моного, но нас интересует контекст для файла журнала, который мы видим в логе SELinux (/var/log/audit/audit.log)

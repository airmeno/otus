...
Borg Backup поддерживает шифрование репозитория. Для шифрования используется опция --encryption (-e). Медоты  шифрования:

* none - не использовать шифрование;
* repokey - парольная фраза, шифрование представляет собой ключевой хэш, алгоритм HMAC-SHA256, сохраняет зашифрованный ключ в ```<REPO_DIR>/ config```
* keyfile - создает ключ шифрования, алгоритм HMAC-SHA256, хранит зашифрованный ключ в ```~/.config/borg/keys/```
* repokey-blake2 - являются аутентифицируемым режимом шифрования, использует BLAKE2b-256 вместо HMAC-SHA256 для аутентификации. Хэш идентификатора блока представляет собой ключевой хэш BLAKE2b-256, сохраняет зашифрованный ключ в ```<REPO_DIR>/ config```


### Настройка стенда

Подготовленный [Vagrantfile](vagrantfile) собирает стед из двух машин: server и client. На server монтирован второй диск размером в 2Гб с точкой монтирования /var/backup. Установлены необходимые пакеты и файлы для BorgServer. BorgServer управляется пользователем borg.
...
Инициализируем с клиента репозитории **BorgRepo** (шифрование через ssh ключ - keyfile). Будет создан ключ  шифрования в ```~/.config/borg/keys/```, вводим парольную фразу и соглашаемся:
```
